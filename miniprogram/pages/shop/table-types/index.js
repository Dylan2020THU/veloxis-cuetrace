...
Page({
  ...
  addOrUpdateType() {
    ...
    if (this.data.editingIdx >= 0) {
      ...
      if (tableTypes[this.data.editingIdx].tableId) {
        newType.tableId = tableTypes[this.data.editingIdx].tableId;
      }
  ...
  save() {
    ...
    data.getStores(storeId).then((stores) => {
      ...
      .then((result) => {
        const savedTableTypes = result && Array.isArray(result.tableTypes)
          ? result.tableTypes
          : this.data.tableTypes;
        this.setData({ tableTypes: savedTableTypes });
        if (getApp().globalData._shopCache) {
          getApp().globalData._shopCache.tableTypes = savedTableTypes;
        }
