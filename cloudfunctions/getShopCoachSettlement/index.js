...
async function fetchSettlements(where) {
  let all = []; let skip = 0; const PS = 100;
  while (true) {
    const r = await db.collection('coach_settlements').where(where).skip(skip).limit(PS).get();
    all = all.concat(r.data);
    if (r.data.length < PS) break;
    skip += PS;
  }
...
function inRange(settlement, range) {
...
exports.main = async (event) => {
  ...
  const settlements = (await fetchSettlements({
    shopOpenid: OPENID,
    coachOpenid: _.in(coachOpenids)
  })).filter((settlement) => inRange(settlement, range));
  ...
  const coaches = coachOpenids.map((openid) => {
    ...
    const settledNet = r2(g.settledNet);
