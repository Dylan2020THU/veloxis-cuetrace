...
async function testCreateRejectsDeletionAndExistingSubscriptionGuards() {
  const variants = [
    { name: 'pending deletion', deletionStatus: 'pending', code: 'ACCOUNT_DELETION_PENDING' },
    { name: 'purging deletion', deletionStatus: 'purging', code: 'ACCOUNT_DELETION_PENDING' },
    { name: 'active subscription', subscriptionStatus: 'active', code: 'ACTIVE_SUBSCRIPTION' },
    { name: 'pending contract', subscriptionStatus: 'pending_contract', code: 'ACTIVE_SUBSCRIPTION' },
    { name: 'cancel required', subscriptionStatus: 'cancel_required', code: 'ACTIVE_SUBSCRIPTION' }
  ];
  for (const variant of variants) {
    const fixture = stateFor(`guard-${variant.name}`, `Guard${variant.name}`);
    fixture.auth.user.deletionStatus = variant.deletionStatus;
    fixture.auth.user.subscriptionStatus = variant.subscriptionStatus;
    ...
    assert.strictEqual(result.code, variant.code, variant.name);
    assert.strictEqual(fixture.state.subscriptions.length, 0, variant.name);
    assert.strictEqual(fakeDb.__operations.adds.length, 0, variant.name);
  }
}

async function testCreateUsesDeterministicIdentityAndAuthoritativeRoles() {
  const fixture = stateFor('role-guard-openid', 'RoleGuardA');
  fixture.auth.user.roles = ['member'];
  fixture.auth.user.role = 'shop';
  fixture.state.users.unshift({
    _id: 'legacy-shop-user',
  ...
  assert.strictEqual(fixture.state.subscriptions.length, 0);
}

async function testCreateAtomicallyWritesSingleSubscriptionGuard() {
  const fixture = stateFor('create-success-openid', 'CreateSuccessA');
  const { fn } = loadCloudFunction(
    'cloudfunctions/createRecurringContract/index.js',
    fixture.auth.binding._openid,
    fixture.state
  ...
  const first = await fn.main(createEvent());
  const subscription = fixture.state.subscriptions[0];

  assert.strictEqual(first.ok, true);
  assert.strictEqual(fixture.state.subscriptions.length, 1);
  assert.strictEqual(subscription._openid, fixture.auth.binding._openid);
  assert.strictEqual(subscription.userId, fixture.auth.user._id);
  assert.strictEqual(subscription.status, 'pending_contract');
  assert.strictEqual(subscription.planId, process.env.PAP_PLAN_ID_MONTH);
  assert(Number.isFinite(subscription.requestSerial));
  assert.strictEqual(fixture.state.users[0].subscriptionStatus, 'pending_contract');
  assert.strictEqual(fixture.state.users[0].subscriptionId, subscription._id);
  assert.strictEqual(first.path, 'pages/index/index');
  assert.strictEqual(first.extraData.path, undefined);
  assert.strictEqual(first.extraData.notify_url, encodeURIComponent(process.env.PAP_CONTRACT_NOTIFY_URL));
  const signingPayload = Object.assign({}, first.extraData, {
    notify_url: process.env.PAP_CONTRACT_NOTIFY_URL
  });
  delete signingPayload.sign;
  assert.strictEqual(first.extraData.sign, signParams(signingPayload, process.env.PAP_SIGN_KEY));

  const second = await fn.main(createEvent());
  assert.strictEqual(second.ok, false);
  assert.strictEqual(second.code, 'ACTIVE_SUBSCRIPTION');
  assert.strictEqual(fixture.state.subscriptions.length, 1);
}

async function testCreateContractCodeDoesNotCollideWithinSameMillisecond() {
  const firstFixture = stateFor('same-millisecond-openid', 'SameMillisecondA');
  const secondFixture = stateFor('same-millisecond-openid', 'SameMillisecondA');
  const firstLoaded = loadCloudFunction(
    'cloudfunctions/createRecurringContract/index.js',
    firstFixture.auth.binding._openid,
    firstFixture.state
  );
  const secondLoaded = loadCloudFunction(
    'cloudfunctions/createRecurringContract/index.js',
    secondFixture.auth.binding._openid,
    secondFixture.state
  );

  const results = await atTime(1000, () => Promise.all([
    firstLoaded.fn.main(createEvent()),
    secondLoaded.fn.main(createEvent())
  ]));

  assert.strictEqual(results[0].ok, true);
  assert.strictEqual(results[1].ok, true);
  assert.notStrictEqual(results[0].extraData.contract_code, results[1].extraData.contract_code);
  results.forEach((result) => {
    assert(/^[0-9A-Za-z]+$/.test(result.extraData.contract_code));
    assert(result.extraData.contract_code.length <= 128);
  });
}

async function testCreateTransactionWriteFailureRollsBackSubscriptionAndGuard() {
  const variants = [
    { collection: 'subscriptions', method: 'add' },
    { collection: 'users', method: 'update' }
  ];
  for (const failure of variants) {
    const fixture = stateFor(`rollback-${failure.collection}`, `Rollback${failure.collection}`);
    ...
    const { fn } = loadCloudFunction(
      ...
      fixture.state,
      {
        failTransactionWrite(operation) {
          return operation.collection === failure.collection && operation.method === failure.method;
        }
      }
    );

    await assert.rejects(() => fn.main(createEvent()), /simulated/);
    assert.deepStrictEqual(fixture.state, before, `${failure.collection}.${failure.method}`);
  }
}

async function testCreateAndDeleteRereadSharedUserGuardInsideTransactions() {
  const createFixture = stateFor('create-conflict-openid', 'CreateConflictA');
  const createLoaded = loadCloudFunction(
    'cloudfunctions/createRecurringContract/index.js',
    createFixture.auth.binding._openid,
    createFixture.state,
    {
      beforeTransaction({ state }) {
        state.users.find((item) => item._id === createFixture.auth.user._id).deletionStatus = 'pending';
      }
    }
  );
  const createResult = await createLoaded.fn.main(createEvent());
  assert.strictEqual(createResult.ok, false);
  assert.strictEqual(createResult.code, 'ACCOUNT_DELETION_PENDING');
  assert.strictEqual(createFixture.state.subscriptions.length, 0);
...
const tests = [
  testCreateRejectsDeletionAndExistingSubscriptionGuards,
  testCreateUsesDeterministicIdentityAndAuthoritativeRoles,
  testCreateAtomicallyWritesSingleSubscriptionGuard,
  testCreateContractCodeDoesNotCollideWithinSameMillisecond,
  testCreateTransactionWriteFailureRollsBackSubscriptionAndGuard,
  testCreateAndDeleteRereadSharedUserGuardInsideTransactions,
