...
function loadCloudFunction(file, openid, state, options) {
  ...
  const fakeCloud = {
    ...
    async deleteFile({ fileList }) {
      ...
      if (options && options.deleteFile) return options.deleteFile(fileList.slice());
  ...
  try {
    ...
    return { fn: require(fnPath), fakeDb, deleteFileCalls };
  } finally {
...
async function testLoginRejectsPurgingAccountWithoutWrites() {
  const fixture = dueState('purging-login-openid', 'PurgingLoginA');
  Object.assign(fixture.state.users[0], {
    deletionStatus: 'purging',
    purgeLeaseId: 'login-lease',
    purgeLeaseExpiresAt: 2000
  });
  Object.assign(fixture.state.account_deletion_requests[0], {
    deletionStatus: 'purging',
  ...
  const result = await atTime(1000, () => fn.main({ role: 'member' }));
...
async function testLoginPendingCancellationRollsBackWhenRequestWriteFails() {
  const fixture = dueState('login-rollback-openid', 'LoginRollbackA');
  fixture.state.users[0].deletionScheduledAt = 2000;
  fixture.state.account_deletion_requests[0].deletionScheduledAt = 2000;
  const before = clone(fixture.state);
  const { fn } = loadCloudFunction(
    'cloudfunctions/login/index.js',
    fixture.auth.binding._openid,
    ...
    {
      failTransactionWrite(operation) {
        return operation.collection === 'account_deletion_requests' && operation.method === 'update';
      }
    }
  );

  await assert.rejects(
    () => atTime(1000, () => fn.main({ role: 'member' })),
    /simulated account_deletion_requests update failure/
  );
  assert.deepStrictEqual(fixture.state, before);
}

async function testLoginRereadsPurgeClaimInsideTransaction() {
  const fixture = dueState('login-claim-race-openid', 'LoginClaimRaceA');
  fixture.state.users[0].deletionScheduledAt = 2000;
  fixture.state.account_deletion_requests[0].deletionScheduledAt = 2000;
  const { fn, fakeDb } = loadCloudFunction(
    'cloudfunctions/login/index.js',
    ...
    fixture.state,
    {
      beforeTransaction({ state }) {
        Object.assign(state.users[0], {
          deletionStatus: 'purging',
          purgeLeaseId: 'login-race-lease',
          purgeLeaseExpiresAt: 3000
        });
        Object.assign(state.account_deletion_requests[0], {
          deletionStatus: 'purging',
          purgeLeaseId: 'login-race-lease',
          purgeLeaseExpiresAt: 3000
        });
  ...
  assert.strictEqual(result.ok, false);
  assert.strictEqual(result.code, 'ACCOUNT_DELETION_LOCKED');
  assert.strictEqual(fixture.state.users[0].deletionStatus, 'purging');
  assert.strictEqual(fixture.state.account_deletion_requests[0].deletionStatus, 'purging');
  assert.strictEqual(fakeDb.__operations.updates.length, 0);
}

async function testLoginRereadsAuthoritativeRoleInsideTransaction() {
  const fixture = dueState('login-role-race-openid', 'LoginRoleRaceA');
  delete fixture.state.users[0].deletionStatus;
  delete fixture.state.users[0].deletionRequestedAt;
  delete fixture.state.users[0].deletionScheduledAt;
  fixture.state.account_deletion_requests = [];
  fixture.state.users[0].roles = ['member', 'coach'];
  const { fn, fakeDb } = loadCloudFunction(
    'cloudfunctions/login/index.js',
    fixture.auth.binding._openid,
    fixture.state,
  ...
  const result = await fn.main({ role: 'coach' });
...
function testExistingDeletionUxAndDeploymentDocumentation() {
  ...
  assert(loginJs.includes("deletionStatus === 'pending'") && loginJs.includes('deletionCanceled'));
...
const tests = [
  ...
  testLoginRejectsPurgingAccountWithoutWrites,
  testLoginPendingCancellationRollsBackWhenRequestWriteFails,
  testLoginRereadsPurgeClaimInsideTransaction,
  testLoginRereadsAuthoritativeRoleInsideTransaction,
