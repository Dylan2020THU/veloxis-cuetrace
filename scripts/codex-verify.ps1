...
try {
  ...
  try {
    ...
    $headResult = Invoke-Git @('rev-parse', '--verify', 'HEAD') -AllowFailure
    if ($headResult.ExitCode -eq 0) {
      $mergeBase = ((Invoke-Git @('merge-base', 'HEAD', $Baseline)).Output | Select-Object -First 1)
      $tracked = (Invoke-Git (@('diff', '--name-only', $mergeBase) + $businessPathspec)).Output
      $diff = Invoke-Git (@('diff', '--check', $mergeBase) + $businessPathspec) -AllowFailure
      if ($diff.ExitCode -ne 0) { $diffOutput = $diff.Output }
    } else {
      $tracked = (Invoke-Git (@('ls-files', '--cached') + $businessPathspec)).Output
      $diffOutput = @()
    }
    $untracked = (Invoke-Git (@('ls-files', '--others', '--exclude-standard') + $businessPathspec)).Output
